...
# Complex Microservices Helm Chart Deployment Script (No Storage)
# This script deploys the comprehensive microservices stack without persistent storage
...
CHART_PATH="."
VALUES_FILE="values-no-storage.yaml"
...
# Deploy the chart
deploy_chart() {
    log_info "Deploying Helm chart with no-storage configuration..."
...
verify_deployment() {
    ...
    # Wait for all pods to be ready (with longer timeout)
    log_info "Waiting for all pods to be ready..."
    kubectl wait --for=condition=ready pod -l app=frontend -n "$NAMESPACE" --timeout=300s || log_warning "Frontend not ready"
    kubectl wait --for=condition=ready pod -l app=backend-api -n "$NAMESPACE" --timeout=300s || log_warning "Backend not ready"
    kubectl wait --for=condition=ready pod -l app=worker -n "$NAMESPACE" --timeout=300s || log_warning "Worker not ready"
    kubectl wait --for=condition=ready pod -l app=prometheus -n "$NAMESPACE" --timeout=300s || log_warning "Prometheus not ready"
    kubectl wait --for=condition=ready pod -l app=postgresql -n "$NAMESPACE" --timeout=300s || log_warning "PostgreSQL not ready"
    kubectl wait --for=condition=ready pod -l app=redis -n "$NAMESPACE" --timeout=300s || log_warning "Redis not ready"
    kubectl wait --for=condition=ready pod -l app=rabbitmq -n "$NAMESPACE" --timeout=300s || log_warning "RabbitMQ not ready"
    ...
    log_info "Checking NodePort services..."
    kubectl get svc -n "$NAMESPACE" | grep NodePort || log_warning "No NodePort services found"
}
...
# Main execution
main() {
    log_info "Starting deployment of complex microservices stack (no storage)..."
    echo ""
